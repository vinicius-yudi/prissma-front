...
describe("triggerFileDownload", () => {
  ...
  it("usa o nome informado como nome do arquivo baixado", () => {
    ...
    // O link é criado e descartado dentro da função, então a única forma de
    // inspecioná-lo é interceptar quem o cria.
    const criado: HTMLAnchorElement[] = []
    const createElement = document.createElement.bind(document)
    vi.spyOn(document, "createElement").mockImplementation((tag: string) => {
      const el = createElement(tag)
      if (tag === "a") criado.push(el as HTMLAnchorElement)
      return el
    })
    ...
    expect(criado[0].download).toBe("memorial descritivo.pdf")
    expect(criado[0].href).toContain("blob:x")
  })
})
