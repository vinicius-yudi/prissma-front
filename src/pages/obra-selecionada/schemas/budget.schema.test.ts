...
function erroDe(schema: { safeParse: (d: unknown) => ReturnType<typeof budgetSchema.safeParse> }, data: unknown, campo: string): string | undefined {
...
describe("budgetSchema", () => {
  ...
  it("exige um número no total planejado", () => {
    expect(erroDe(budgetSchema, { plannedTotal: "1000" }, "plannedTotal")).toBe("Valor obrigatório")
  })
  ...
  it("aceita zero e rejeita negativo", () => {
    ...
    expect(erroDe(budgetSchema, { plannedTotal: -1 }, "plannedTotal")).toBe("Valor deve ser positivo")
  })

  it("rejeita valor acima do limite do backend", () => {
    expect(erroDe(budgetSchema, { plannedTotal: 1e15 }, "plannedTotal")).toBe("Valor acima do limite")
  })
  ...
  it("limita a descrição a 255 caracteres", () => {
    ...
    expect(erroDe(budgetSchema, { description: "x".repeat(256), plannedTotal: 1 }, "description")).toBe(
...
describe("budgetItemSchema", () => {
  ...
  it("exige categoria e descrição", () => {
    ...
    expect(erroDe(budgetItemSchema, data, "category")).toBe("Categoria obrigatória")
    expect(erroDe(budgetItemSchema, data, "description")).toBe("Descrição obrigatória")
  })

  it("limita a categoria a 100 caracteres", () => {
    const data = { category: "x".repeat(101), description: "ok", plannedAmount: 1 }

    expect(erroDe(budgetItemSchema, data, "category")).toBe("Máximo 100 caracteres")
  })

  it("tem defaults que só falham nos campos obrigatórios", () => {
    const result = budgetItemSchema.safeParse(BUDGET_ITEM_FORM_DEFAULTS)

    expect(result.success).toBe(false)
    if (result.success) return
    expect(Object.keys(result.error.flatten().fieldErrors).sort()).toEqual(["category", "description"])
  })
})
...
describe("expenseSchema", () => {
  ...
  it("rejeita valor zero", () => {
    expect(erroDe(expenseSchema, { ...valida, amount: 0 }, "amount")).toBe("Valor deve ser maior que zero")
  })

  it("exige data e valida o formato", () => {
    expect(erroDe(expenseSchema, { ...valida, spentAt: "" }, "spentAt")).toBe("Data obrigatória")
    expect(erroDe(expenseSchema, { ...valida, spentAt: "15/03/2026" }, "spentAt")).toBe("Data inválida")
  })
  ...
  it("exige http ou https no recibo", () => {
    expect(erroDe(expenseSchema, { ...valida, receiptUrl: "recibos.exemplo/1" }, "receiptUrl")).toBe(
  ...
  it("tem defaults que só falham nos campos obrigatórios", () => {
    const result = expenseSchema.safeParse(EXPENSE_FORM_DEFAULTS)
