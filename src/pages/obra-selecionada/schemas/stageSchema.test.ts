...
function erroDeEtapa(data: unknown, campo: string): string | undefined {
...
describe("stageSchema", () => {
  ...
  it("exige nome", () => {
    expect(erroDeEtapa({ ...etapaValida, name: "" }, "name")).toBe("Nome obrigatório")
  })
  ...
  it("exige ordem inteira e positiva", () => {
    expect(erroDeEtapa({ ...etapaValida, displayOrder: 0 }, "displayOrder")).toBe("Ordem inválida")
  ...
  it("exige as duas datas planejadas", () => {
    expect(erroDeEtapa({ ...etapaValida, plannedStartDate: "" }, "plannedStartDate")).toBe(
      "Data de início planejada é obrigatória",
    )
    expect(erroDeEtapa({ ...etapaValida, plannedEndDate: "" }, "plannedEndDate")).toBe(
      "Data de término planejada é obrigatória",
    )
  })

  it("rejeita término anterior ao início", () => {
    expect(erroDeEtapa({ ...etapaValida, plannedEndDate: "2026-02-01" }, "plannedEndDate")).toBe(
  ...
  it("tem defaults que só falham nas datas ainda não preenchidas", () => {
    const result = stageSchema.safeParse(STAGE_FORM_DEFAULTS)
...
describe("taskSchema", () => {
  ...
  function erroDeTarefa(data: unknown, campo: string): string | undefined {
  ...
  it("exige título e descrição", () => {
    expect(erroDeTarefa({ ...tarefaValida, title: "" }, "title")).toBeTruthy()
    expect(erroDeTarefa({ ...tarefaValida, description: "" }, "description")).toBeTruthy()
  })
  ...
  it("exige um responsável de verdade", () => {
    expect(erroDeTarefa({ ...tarefaValida, assigneeUserId: 0 }, "assigneeUserId")).toBeTruthy()
    expect(erroDeTarefa({ ...tarefaValida, assigneeUserId: undefined }, "assigneeUserId")).toBeTruthy()
  })

  it("rejeita término anterior ao início e aceita no mesmo dia", () => {
    expect(erroDeTarefa({ ...tarefaValida, plannedEndDate: "2026-02-01" }, "plannedEndDate")).toBeTruthy()
    expect(
