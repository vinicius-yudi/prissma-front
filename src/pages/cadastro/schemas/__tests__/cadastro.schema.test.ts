...
describe.each(SCHEMAS)("cadastro de %s", (_perfil, schema) => {
  ...
  it("exige nome com pelo menos 3 caracteres", () => {
    expect(fieldError(schema, { ...valido, name: "Jo" }, "name")).toBe("O nome deve conter pelo menos 3 caracteres")
  })

  it("exige e-mail em formato válido", () => {
    expect(fieldError(schema, { ...valido, email: "ana-construtora" }, "email")).toBe("Formato de e-mail inválido")
  })

  it("exige senha com pelo menos 6 caracteres", () => {
    expect(fieldError(schema, { ...valido, password: "Ob@1", confirmPassword: "Ob@1" }, "password")).toBe(
  ...
  it("cobra maiúscula, minúscula, número e símbolo separadamente", () => {
    ...
    for (const [password, mensagem] of casos) {
      expect(fieldError(schema, { ...valido, password, confirmPassword: password }, "password"), password).toBe(
        mensagem,
      )
    }
  })
  ...
  it("exige a confirmação", () => {
    expect(fieldError(schema, { ...valido, confirmPassword: "" }, "confirmPassword")).toBe(
  ...
  it("acusa senhas diferentes no campo de confirmação", () => {
    expect(fieldError(schema, { ...valido, confirmPassword: "Outra@2026" }, "confirmPassword")).toBe(
